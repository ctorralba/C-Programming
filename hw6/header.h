...
//Constants
//*******************************
const short LEAST_NUM_DISK = 1;	//used to check the disk num
const short GREAT_NUM_DISK = 5;	//used to check disk num
const short NOT_ASCII = 65;	//converts char input to not an ASCII (A=0)
const short CYCLE_BACK_NUM = 26;//if the shift value makes a character over
				//the letter 'Z' this is used to revert back
				//'A'
const short D1_ADD = 3;		//Disk 1 add number
const short D2_ADD = 17;	//Disk 2 add number
const short D12_MOD = 26;	//Disk 1 & 2 mod number
const short D12_FINAL_ADD = 65;	//Disk 1 & 2 add number at the end
const short D45_EVENCHECK = 2;	//used to check if an ASCII value is even
const short D4_ISEVEN = 0;	//used to determine that a char is even
const short D45_MODIFIER = 1;	//used to add or subtract to swap char
const short D5_MULTIPLE3 = 3;	//Disk 5 used to check if multiple of 3
				// and add if multiple of 3
/******************************
//Function Prototypes
